    ...
    public static class CalcImpl2Patch
    {
        ...
        public static bool Prefix(ref ZoneBlock __instance, ref ZoneBlock other, ref ulong valid, ref ulong shared, float minX, float minZ, float maxX, float maxZ)
        {
            ...
            for (int row = 0; row < rowCount; ++row)
            {
                ...
                {
                    for (int column = 0; (long)column < ZoneBlock.COLUMN_COUNT && ((long)valid & 1L << (row << 3 | column)) != 0L; ++column)
                    {
                            ...
                            otherZoneBlockQuad.c + otherColumnDirection + otherRowDirection, otherZoneBlockQuad.d - otherColumnDirection + otherRowDirection, cellMiddlePos))
                        {
                            ...
                            for (int otherRow = 0; otherRow < otherRowCount && cellIsValid; ++otherRow)
                            {
                                ...
                                for (int otherColumn = 0; (long)otherColumn < ZoneBlock.COLUMN_COUNT && cellIsValid; ++otherColumn)
                                {
